...
class Learner(BaseLearner):
    ...
    def incremental_train(self, data_manager):
        ...
        if ca_epochs > 0:
            ...
            if self._cur_task > 0:
                ...
                # Re-save fc after CA updates the classifier
    ...
    def _compute_class_mean(self, data_manager):
        ...
        Old-class statistics are kept from previous tasks (cached).  Only
        classes in [_known_classes, _total_classes) are freshly computed
        """
        net = self._network.module if isinstance(self._network, nn.DataParallel) else self._network
        feat_dim = net.feature_dim

        # Initialise or expand storage (preserve old-class cache)
        ...
        # Only compute for the NEW classes of the current task
        for class_idx in range(self._known_classes, self._total_classes):
            # Training data with test-mode transform (no augmentation)
            ...
            # Covariance + diagonal jitter for numerical stability
            ...
            # Cholesky check; add more jitter if needed
            try:
    ...
    def _stage2_compact_classifier(self):
        """CA stage 2: retrain only the classifier using sampled pseudo-features.

        Pseudo-features are drawn from per-class Gaussian distributions
        estimated by ``_compute_class_mean``.  A task-aware mean decay is
        applied following the InfLoRA-CA design.
        """
        ...
        # Build per-task class ranges (supports variable task sizes)
        task_sizes = []
        ...
        for epoch in range(ca_epochs):
            ...
            for c_id in range(self._total_classes):
                # Map class id -> task id using actual task_sizes
                _acc = 0
                t_id = 0
            ...
            for i in range(0, sampled_data.size(0), batch_size):
                ...
                if logit_norm is not None:
                    # Decoupled logit normalization (per-task-block norms)
                    norms = []
                    idx = 0
